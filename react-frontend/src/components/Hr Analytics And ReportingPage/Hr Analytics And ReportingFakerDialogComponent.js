...
import Hr Analytics And ReportingFakerFactory from "./Hr Analytics And ReportingFakerFactory";
...
const Hr Analytics And ReportingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Hr Analytics And ReportingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Hr Analytics And Reporting").create(elem));
...
export default connect(null, mapDispatch)(Hr Analytics And ReportingFakerDialogComponent);

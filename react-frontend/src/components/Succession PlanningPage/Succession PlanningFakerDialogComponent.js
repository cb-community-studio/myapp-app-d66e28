...
import Succession PlanningFakerFactory from "./Succession PlanningFakerFactory";
...
const Succession PlanningFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Succession PlanningFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Succession Planning").create(elem));
...
export default connect(null, mapDispatch)(Succession PlanningFakerDialogComponent);

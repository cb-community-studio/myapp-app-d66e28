...
import Recruitment And OnboardingFakerFactory from "./Recruitment And OnboardingFakerFactory";
...
const Recruitment And OnboardingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Recruitment And OnboardingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Recruitment And Onboarding").create(elem));
...
export default connect(null, mapDispatch)(Recruitment And OnboardingFakerDialogComponent);

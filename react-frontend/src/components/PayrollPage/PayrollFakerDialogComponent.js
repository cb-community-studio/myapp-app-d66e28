...
import PayrollFakerFactory from "./PayrollFakerFactory";
...
const PayrollFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = PayrollFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Payroll").create(elem));
...
export default connect(null, mapDispatch)(PayrollFakerDialogComponent);

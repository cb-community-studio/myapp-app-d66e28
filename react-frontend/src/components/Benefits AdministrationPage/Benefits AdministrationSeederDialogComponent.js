...
import seederData from "./Benefits Administration.seeder.json";
...
const Benefits AdministrationSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Benefits Administration").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Benefits AdministrationSeederDialogComponent);

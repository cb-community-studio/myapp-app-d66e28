...
import seederData from "./Benefits Open Enrollment.seeder.json";
...
const Benefits Open EnrollmentSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Benefits Open Enrollment").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Benefits Open EnrollmentSeederDialogComponent);

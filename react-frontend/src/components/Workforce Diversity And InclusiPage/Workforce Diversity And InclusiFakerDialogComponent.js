...
import Workforce Diversity And InclusiFakerFactory from "./Workforce Diversity And InclusiFakerFactory";
...
const Workforce Diversity And InclusiFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Workforce Diversity And InclusiFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Workforce Diversity And Inclusi").create(elem));
...
export default connect(null, mapDispatch)(Workforce Diversity And InclusiFakerDialogComponent);

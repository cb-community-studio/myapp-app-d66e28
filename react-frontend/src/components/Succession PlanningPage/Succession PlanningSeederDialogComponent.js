...
import seederData from "./Succession Planning.seeder.json";
...
const Succession PlanningSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Succession Planning").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Succession PlanningSeederDialogComponent);

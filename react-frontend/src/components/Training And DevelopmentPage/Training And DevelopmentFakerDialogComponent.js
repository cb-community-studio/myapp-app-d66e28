...
import Training And DevelopmentFakerFactory from "./Training And DevelopmentFakerFactory";
...
const Training And DevelopmentFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Training And DevelopmentFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Training And Development").create(elem));
...
export default connect(null, mapDispatch)(Training And DevelopmentFakerDialogComponent);

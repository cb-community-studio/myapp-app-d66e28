...
import Employee RelationsFakerFactory from "./Employee RelationsFakerFactory";
...
const Employee RelationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Employee RelationsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Employee Relations").create(elem));
...
export default connect(null, mapDispatch)(Employee RelationsFakerDialogComponent);

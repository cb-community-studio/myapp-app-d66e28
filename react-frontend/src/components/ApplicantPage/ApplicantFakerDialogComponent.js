...
import ApplicantFakerFactory from "./ApplicantFakerFactory";
...
const ApplicantFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = ApplicantFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Applicant").create(elem));
...
export default connect(null, mapDispatch)(ApplicantFakerDialogComponent);

...
import Time TrackingFakerFactory from "./Time TrackingFakerFactory";
...
const Time TrackingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Time TrackingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Time Tracking").create(elem));
...
export default connect(null, mapDispatch)(Time TrackingFakerDialogComponent);

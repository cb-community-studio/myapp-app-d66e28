...
import Attendance And Leave ManagementFakerFactory from "./Attendance And Leave ManagementFakerFactory";
...
const Attendance And Leave ManagementFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Attendance And Leave ManagementFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Attendance And Leave Management").create(elem));
...
export default connect(null, mapDispatch)(Attendance And Leave ManagementFakerDialogComponent);

...
import seederData from "./Employee Surveys And Feedback.seeder.json";
...
const Employee Surveys And FeedbackSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Employee Surveys And Feedback").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Employee Surveys And FeedbackSeederDialogComponent);

...
import seederData from "./Employee Relations.seeder.json";
...
const Employee RelationsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Employee Relations").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Employee RelationsSeederDialogComponent);

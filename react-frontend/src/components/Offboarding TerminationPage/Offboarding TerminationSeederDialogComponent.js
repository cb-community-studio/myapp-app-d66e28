...
import seederData from "./Offboarding Termination.seeder.json";
...
const Offboarding TerminationSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Offboarding Termination").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Offboarding TerminationSeederDialogComponent);

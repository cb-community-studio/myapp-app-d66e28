...
import Performance ReviewFakerFactory from "./Performance ReviewFakerFactory";
...
const Performance ReviewFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Performance ReviewFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Performance Review").create(elem));
...
export default connect(null, mapDispatch)(Performance ReviewFakerDialogComponent);

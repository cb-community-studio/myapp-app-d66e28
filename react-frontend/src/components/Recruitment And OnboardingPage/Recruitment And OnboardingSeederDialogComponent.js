...
import seederData from "./Recruitment And Onboarding.seeder.json";
...
const Recruitment And OnboardingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Recruitment And Onboarding").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Recruitment And OnboardingSeederDialogComponent);

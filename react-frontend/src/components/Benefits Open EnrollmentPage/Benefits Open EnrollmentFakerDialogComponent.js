...
import Benefits Open EnrollmentFakerFactory from "./Benefits Open EnrollmentFakerFactory";
...
const Benefits Open EnrollmentFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Benefits Open EnrollmentFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Benefits Open Enrollment").create(elem));
...
export default connect(null, mapDispatch)(Benefits Open EnrollmentFakerDialogComponent);

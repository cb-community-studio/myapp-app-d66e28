...
const Performance ReviewCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        setLoading(true);
        try {
            const result = await client.service("Performance Review").patch(_entity._id, _data);
            props.onHide();
    ...
    return (
        <Dialog header="Edit Info" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="Performance Review-edit-dialog-component">
                <div>
...
export default connect(null, mapDispatch)(Performance ReviewCreateDialogComponent);

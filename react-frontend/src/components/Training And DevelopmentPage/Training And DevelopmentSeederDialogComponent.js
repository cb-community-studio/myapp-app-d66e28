...
import seederData from "./Training And Development.seeder.json";
...
const Training And DevelopmentSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Training And Development").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Training And DevelopmentSeederDialogComponent);

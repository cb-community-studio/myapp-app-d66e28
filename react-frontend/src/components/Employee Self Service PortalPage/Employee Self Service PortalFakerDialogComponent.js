...
import Employee Self Service PortalFakerFactory from "./Employee Self Service PortalFakerFactory";
...
const Employee Self Service PortalFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Employee Self Service PortalFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Employee Self Service Portal").create(elem));
...
export default connect(null, mapDispatch)(Employee Self Service PortalFakerDialogComponent);

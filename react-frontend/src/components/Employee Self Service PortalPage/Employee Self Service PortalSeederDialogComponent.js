...
import seederData from "./Employee Self Service Portal.seeder.json";
...
const Employee Self Service PortalSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Employee Self Service Portal").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Employee Self Service PortalSeederDialogComponent);

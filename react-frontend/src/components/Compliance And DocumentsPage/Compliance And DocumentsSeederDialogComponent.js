...
import seederData from "./Compliance And Documents.seeder.json";
...
const Compliance And DocumentsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Compliance And Documents").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Compliance And DocumentsSeederDialogComponent);

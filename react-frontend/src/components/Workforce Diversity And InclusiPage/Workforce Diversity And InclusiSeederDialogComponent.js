...
import seederData from "./Workforce Diversity And Inclusi.seeder.json";
...
const Workforce Diversity And InclusiSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Workforce Diversity And Inclusi").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Workforce Diversity And InclusiSeederDialogComponent);

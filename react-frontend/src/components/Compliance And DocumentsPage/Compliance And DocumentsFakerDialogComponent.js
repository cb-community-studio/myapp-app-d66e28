...
import Compliance And DocumentsFakerFactory from "./Compliance And DocumentsFakerFactory";
...
const Compliance And DocumentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Compliance And DocumentsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Compliance And Documents").create(elem));
...
export default connect(null, mapDispatch)(Compliance And DocumentsFakerDialogComponent);

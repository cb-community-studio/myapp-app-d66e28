...
import seederData from "./Hr Budgeting And Cost Tracking.seeder.json";
...
const Hr Budgeting And Cost TrackingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Hr Budgeting And Cost Tracking").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Hr Budgeting And Cost TrackingSeederDialogComponent);

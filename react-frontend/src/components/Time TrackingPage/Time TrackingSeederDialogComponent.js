...
import seederData from "./Time Tracking.seeder.json";
...
const Time TrackingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Time Tracking").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Time TrackingSeederDialogComponent);

...
import seederData from "./Hr Analytics And Reporting.seeder.json";
...
const Hr Analytics And ReportingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Hr Analytics And Reporting").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Hr Analytics And ReportingSeederDialogComponent);

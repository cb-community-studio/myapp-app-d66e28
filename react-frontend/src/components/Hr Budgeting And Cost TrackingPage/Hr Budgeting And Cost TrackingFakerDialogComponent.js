...
import Hr Budgeting And Cost TrackingFakerFactory from "./Hr Budgeting And Cost TrackingFakerFactory";
...
const Hr Budgeting And Cost TrackingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Hr Budgeting And Cost TrackingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Hr Budgeting And Cost Tracking").create(elem));
...
export default connect(null, mapDispatch)(Hr Budgeting And Cost TrackingFakerDialogComponent);

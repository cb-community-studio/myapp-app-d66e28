...
import seederData from "./Performance Review.seeder.json";
...
const Performance ReviewSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Performance Review").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Performance ReviewSeederDialogComponent);

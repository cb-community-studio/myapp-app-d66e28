...
import seederData from "./Attendance And Leave Management.seeder.json";
...
const Attendance And Leave ManagementSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("Attendance And Leave Management").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Attendance And Leave ManagementSeederDialogComponent);

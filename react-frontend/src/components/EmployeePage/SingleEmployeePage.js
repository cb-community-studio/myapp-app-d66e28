...
const SingleEmployeePage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("Employee")
            .get(urlParams.singleEmployeeId, { query: { $populate: ["employee"] }})
            .then((res) => {
                set_entity(res || {});
                const users = Array.isArray(res.employee)
            ? res.employee.map((elem) => ({ _id: elem._id, name: elem.name }))
            : res.employee
                ? [{ _id: res.employee._id, name: res.employee.name }]
                : [];
        setusers(users);
                setusers(res?.employee?.map((elem) => ({ _id: elem._id, name: elem.name })) || []);
            })
            .catch((error) => {
                console.log({ error });

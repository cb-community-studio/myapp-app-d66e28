...
import Employee Surveys And FeedbackFakerFactory from "./Employee Surveys And FeedbackFakerFactory";
...
const Employee Surveys And FeedbackFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Employee Surveys And FeedbackFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Employee Surveys And Feedback").create(elem));
...
export default connect(null, mapDispatch)(Employee Surveys And FeedbackFakerDialogComponent);

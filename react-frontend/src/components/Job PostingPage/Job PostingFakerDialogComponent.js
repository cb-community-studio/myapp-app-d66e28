...
import Job PostingFakerFactory from "./Job PostingFakerFactory";
...
const Job PostingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = Job PostingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("Job Posting").create(elem));
...
export default connect(null, mapDispatch)(Job PostingFakerDialogComponent);
